namespace NTechDevelpers.Utilities
{
    ...
    public static class Preconditions
    {
        ...
        /// <summary>
        /// Checks the file has write permission?
        /// </summary>
        /// <param name="value">The value to check.</param>
        /// <param name="name">The name of the parameter the value is taken from.</param>
        /// <exception cref="Exception"></exception>
        public static void CheckFileHasWritePermission(string value, string name)
        {
            CheckNotNull(value, name);
            CheckNotBlank(name, NameParamName);
            CheckNotBlank(value, name);

            if (!FilePermissionChecker.HasWritePermission(value, name))
            {
                var errorMessage = string.Format(CultureInfo.CurrentCulture, "Access to the path '{0}' is denied.", value);
                Logger.Error(errorMessage);
                throw new Exception(errorMessage);
            }
        }
        ...
        /// <summary>
        /// Checks the file has read permission?
        /// </summary>
        /// <param name="value">The value to check.</param>
        /// <param name="name">The name of the parameter the value is taken from.</param>
        /// <exception cref="Exception"></exception>
        public static void CheckFileHasReadPermission(string value, string name)
        {
            CheckNotNull(value, name);
            CheckNotBlank(name, NameParamName);
            CheckNotBlank(value, name);

            if (!FilePermissionChecker.HasReadPermission(value, name))
            {
                var errorMessage = string.Format(CultureInfo.CurrentCulture, "Access to the path '{0}' is denied.", value);
                Logger.Error(errorMessage);
                throw new Exception(errorMessage);
